    ...
    * ``B_pages[module_name][layer]``: ``[total_pages, output_dim, PAGE_RANK_SIZE]``
    ...
    def evict_pages(
        ...
        for p in candidates:
            ...
            self.free_page_indices.add(p)
            evicted.append(p)
            self.eviction_events = getattr(self, "eviction_events", 0) + 1
            logger.info(
                "LoRA page evicted (I7): phys_page=%d adapter=%s logic_idx=%d "
                "total_evictions=%d bytes_paged_in=%d",
                p,
                uid,
                logic_idx if uid is not None else -1,
                self.eviction_events,
                getattr(self, "bytes_paged_in", 0),
            )
    ...
    def page_in(self, uid: str, logic_page_idx: int) -> int:
        ...
        if needed).  Returns -1 instead of asserting when the pool is
    ...
    def get_eviction_stats(self) -> dict:
        """Return eviction statistics for I7 metric collection."""
        bytes_per_page = self._compute_page_bytes()
        self.total_bytes_evicted = self.eviction_events * bytes_per_page
        return {
            "eviction_events": self.eviction_events,
            "total_bytes_evicted": self.total_bytes_evicted,
            # Actual host->device bytes scattered (real swap-in I/O).
            "bytes_paged_in": self.bytes_paged_in,
            "bytes_per_page": bytes_per_page,
            "total_pages": self.total_pages,
            "free_pages": len(self.free_page_indices),
            "used_pages": self.total_pages - len(self.free_page_indices),
        }
            ...
            for ci in range(c):
                ...
                if weight_2d is not None:
                    ...
                    if w.shape[-1] > page_hidden:
                        hid_offset = self.tp_rank * (w.shape[-1] // tp)
                    ...
                    self.bytes_paged_in += w.numel() * w.element_size()
                else:
        ...
        for logic_idx, phys in zip(logic_page_indices, phys_pages):
            ...
            if weight_2d is not None:
                ...
                if w_src.shape[0] > page_output_dim:
                    out_offset = self.tp_rank * (w_src.shape[0] // tp)
    ...
    def ensure_adapter_ready(
        ...
        * If fully resident → no-op.
        ...
        if uid not in self.page_table:
            ...
            bytes_before = self.bytes_paged_in
            ...
            logger.info(
                "LoRA page-in (cold): uid=%s total_evictions=%d bytes_paged_in=%d",
                uid,
                self.eviction_events,
                self.bytes_paged_in,
            )
        ...
        if missing:
            ...
            logger.info(
                "LoRA page-in (reload): uid=%s missing=%d total_evictions=%d bytes_paged_in=%d",
                uid,
                needed,
                self.eviction_events,
                self.bytes_paged_in,
